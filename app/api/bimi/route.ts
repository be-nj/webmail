...
// has no usable logo. `verified: true` is the logo inside the domain's verified
// mark certificate, after it checked out against our own roots (lib/vmc.ts).
// `verified: false` is the picture at the record's l= URL: any domain can point
// that at someone else's logo, so the client shows it only for a Trusted
// Sender of that domain (docs/adr/0001).
...
function answer(logo: Logo | null, maxAgeSeconds: number) {
  return NextResponse.json({ svg: logo?.svg ?? null, verified: logo?.verified ?? false }, {
    headers: {
